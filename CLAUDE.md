...
Shir targets **one physical iPhone, sideloaded**. It is not going to the App Store.
...
4. Search through the official Data API v3 with a user-supplied key, rather than InnerTube.
   `git show c7fc009:ShirKit/Sources/ShirKit/YouTube/YouTubeSearchClient.swift` has the
...
- **No key is needed.** The `key` parameter is ignored — a deliberately bogus key returns
  ...
  sending it. So Shir sends none, which also removes the "whose key is this?" question.
- **Search needs no bot attestation.** PoToken and BotGuard gate `/youtubei/v1/player`,
  ...
  stream extraction does not — and it is why Shir does not extract streams.
...
**Test the injected JavaScript in `ShirKit` via `JSContext`.** `JavaScriptCore` is not a UI
framework, so it does not violate ShirKit's no-UI-imports rule. The ad-strip logic is a
...
- `docs/shir-architecture.html` documents the **pre-fork** architecture — IFrame embed,
