...
/// Tests `MediaSession.js`, which is what makes the iOS lock screen show Riff's
/// track and route its next button into Riff's queue.
///
...
final class MediaSessionScriptTests: XCTestCase {
    ...
    private func context() throws -> JSContext {
        ...
        context.evaluateScript(
            ...
            var window = this;
            window.top = window;
            window.webkit = { messageHandlers: { riff: { postMessage: function (m) {
            ...
            window.__riff = { play: function () {}, pause: function () {}, seek: function () {} };
            """
        )
    ...
    /// wins, the lock screen advances YouTube's playlist instead of Riff's queue.
    ...
    /// Actions Riff does not claim should still reach the page — there is no
    ...
    func testUnownedActionsStillPassThrough() throws {
        ...
        XCTAssertTrue(nativeHandlerExists("skipad", in: context),
                      "only the actions Riff owns should be blocked")
    }
    ...
    func testThePageCannotOverwriteMetadata() throws {
        ...
        context.evaluateScript(
            """
            window.__riffMedia.setMetadata('Real Song', 'Real Artist', '', 'https://i.ytimg.com/vi/x/hqdefault.jpg');
    ...
    func testPressingNextReportsToSwiftWithoutActingLocally() throws {
        ...
        context.evaluateScript(
            ...
            window.__riff.play = function () { window.__playCalled = true; };
            window.__native.handlers['nexttrack']();
            """
        )
        ...
        XCTAssertEqual(context.evaluateScript("window.__playCalled")?.toBool(), false,
                       "next must be Riff's queue advancing, not the page's")
    }
    ...
    func testMetadataReachesTheNativeLayer() throws {
        ...
        context.evaluateScript(
            "window.__riffMedia.setMetadata('Song', 'Artist', '', 'https://i.ytimg.com/vi/x/hqdefault.jpg');"
        )
    ...
    func testPositionStateIsPushedForTheScrubber() throws {
        let context = try context()
        context.evaluateScript("window.__riffMedia.pushPosition();")
