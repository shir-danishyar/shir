# Ad-free YouTube playback in Riff — design
...
Riff plays YouTube through the official IFrame Player today: ads intact, player visible,
...
- Riff's own playlists and queue drive playback, not YouTube's

`RiffKit` does not change. The domain layer — library, playlists, `PlaybackQueue`,
...
A throwaway target. Not in the Riff tree, deleted when done. One view, a `WKWebView` on
...
fall back to driving YouTube's own playlist via `nextVideo()` and reconciling Riff's queue
to it — worse, but survivable.

There is reason for cautious optimism: Brave delegates ordering to YouTube's playlist page,
so every transition is a navigation. Riff owns the queue, so a transition is one call into
...
Exactly one component's internals: `Riff/Playback/YouTubePlayerEngine.swift`. It stops
...
and every `RiffKit` type are untouched. Local file playback is unaffected.
...
`AdStrip.js` is a pure function — JSON in, JSON out. Test it in `RiffKit` by loading the
...
`JavaScriptCore` is not a UI framework, so this does not violate RiffKit's no-UI-imports
...
- `RiffKit`'s existing 51 tests must continue to pass untouched. If a change to this layer
...
8. Regenerate `docs/riff-architecture.html`, which will be stale from step 4 onward.
...
- How should Riff behave during an upstream breakage — play with ads, or refuse and fall
