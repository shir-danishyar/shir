...
Throwaway. Delete `spike/` once it has answered its question; nothing in Riff
depends on it.
...
```bash
xcrun simctl launch <device-id> com.shirhussain.riff.spike -autoadvance
```
...
a track change — which is the mechanism criterion 1 needs. Riff owning the queue
...
be touching YouTube's interface at all** — which is how Musi worked, and how
Riff is meant to work:
...
Riff already has the other half: `RiffKit`'s `YouTubeSearchClient` searches with
