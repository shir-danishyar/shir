...
(function () {
  ...
  function report(message) {
    try {
      window.webkit.messageHandlers.spike.postMessage({ kind: 'log', text: message });
    } catch (e) {
      /* handler not registered — spike still works, we just lose the log line */
    }
  }
  ...
  XMLHttpRequest.prototype.send = function () {
    ...
    request.addEventListener('readystatechange', function () {
      ...
      if (!isTargetURL(request.__spikeURL || '')) return;
